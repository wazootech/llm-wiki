...
The **`wiki-sync`** skill maintains *code wikis* (a repo's `docs/` folder, e.g. `wazootech/sparql-engine`) — the Git-anchored delta process that keeps documentation truthful to source after code changes: anchor at `docs/.sync-base`, diff `origin/main` forward, rebuild symbol lines from `deno doc --json`, verify counts by running the test runners, check file inventory with `git ls-tree`, edit only the affected pages, validate, and land. Canonical file: [`skills/wiki-sync/SKILL.md`](https://github.com/wazootech/wiki/blob/main/skills/wiki-sync/SKILL.md).
